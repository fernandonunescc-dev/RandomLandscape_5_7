...
/**
 * Stage 8: Cave generation settings — 3D noise-based cave carving beneath the terrain surface.
 *
 * Caves are generated by sampling 3D FBM noise in a volumetric grid underneath
 * the terrain surface. Voxels where the noise exceeds CaveThreshold are marked
 * as air (cave interior). The resulting density field is converted to triangle
 * meshes via Marching Cubes and added as additional ProceduralMeshComponent
 * sections alongside the heightmap terrain.
 *
 * Caves only form where LandMask == 1 and below the surface elevation.
 */
USTRUCT(BlueprintType)
struct RANDOMLANDSCAPE_5_7_API FCaveSettings
{
	GENERATED_BODY()

	/** Seed override (0 = derive from global seed) */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Cave", meta = (ClampMin = "0", Tooltip = "Per-stage random seed. Leave at 0 to derive automatically from the global world seed."))
	int32 Seed = 0;

	/** Enable cave generation. When false, the cave stage is skipped entirely. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Cave", meta = (Tooltip = "Master toggle for cave generation. When disabled, no cave meshes are produced."))
	bool bEnabled = true;

	/** 3D noise threshold above which a voxel is considered cave air.
	 *  Lower values create more caves; higher values create fewer, smaller caves. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Cave", meta = (ClampMin = "0.0", ClampMax = "1.0", Tooltip = "3D FBM noise threshold above which a voxel is classified as cave interior (air). Lower values produce more cave volume; higher values produce sparser, smaller caves."))
	float CaveThreshold = 0.55f;

	/** 3D noise frequency controlling cave feature size.
	 *  Low values = large caverns, high values = narrow tunnels. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Cave", meta = (ClampMin = "0.5", ClampMax = "20.0", Tooltip = "Frequency of the 3D FBM noise used to carve caves. Low values produce large, open caverns; high values produce narrow, winding tunnels."))
	float CaveScale = 3.0f;

	/** Number of FBM octaves for cave noise detail. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Cave", meta = (ClampMin = "1", ClampMax = "6", Tooltip = "Number of FBM octaves layered for cave noise. More octaves add finer detail to cave walls and passages."))
	int32 CaveOctaves = 4;

	/** FBM persistence (amplitude falloff per octave) for cave noise. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Cave", meta = (ClampMin = "0.1", ClampMax = "0.9", Tooltip = "Persistence (amplitude decay) per FBM octave. Higher values make successive octaves contribute more, producing rougher cave walls."))
	float CavePersistence = 0.5f;

	/** Maximum cave depth in metres below the terrain surface. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Cave", meta = (ClampMin = "5.0", ClampMax = "500.0", Tooltip = "Maximum depth in metres below the terrain surface where caves can form. Deeper values allow cave networks further underground."))
	float MaxCaveDepthMeters = 40.0f;

	/** Number of vertical depth layers in the 3D sampling grid.
	 *  More layers = finer vertical resolution at higher memory cost. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Cave", meta = (ClampMin = "8", ClampMax = "64", Tooltip = "Number of vertical slices in the 3D voxel grid. More layers produce smoother cave geometry at the cost of memory and compute time."))
	int32 DepthLayers = 16;

	/** Horizontal resolution of the cave voxel grid.
	 *  0 = match TextureResolution. Lower values speed up generation at the cost of detail. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Cave", meta = (ClampMin = "0", ClampMax = "1024", Tooltip = "Horizontal resolution of the cave 3D grid. 0 means use the same resolution as TextureResolution. Lower values reduce memory and generation time."))
	int32 CaveResolution = 0;

	/** Minimum surface elevation for cave formation (normalised [0,1]).
	 *  Prevents caves in very low coastal areas where they would be below sea level. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Cave", meta = (ClampMin = "0.0", ClampMax = "0.5", Tooltip = "Minimum normalised surface elevation required for caves to form below a pixel. Prevents underwater cave openings in coastal lowlands."))
	float MinSurfaceElevation = 0.05f;
};
