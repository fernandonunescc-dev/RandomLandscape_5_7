...
// Implementation of landmass texture generation using FastNoise2 Simplex FBM noise
...
void ULandmassGenerator::Initialize(const FLandmassSettings& InSettings)
{
	...
	// === Build FN2 Simplex node for per-sample noise ===
	// Built once here (main thread), then used read-only from ParallelFor workers.
	// SmartNode is immutable after construction — GenPositionArray2D is thread-safe.
	SimplexNode = FastNoise::New<FastNoise::Simplex>();
...
//------------------------------------------------------------------------------
// Noise2D: Simplex noise via pre-built FN2 node.
// The SimplexNode is built once in Initialize() and is immutable —
// GenPositionArray2D is safe to call from ParallelFor worker threads.
// Returns values approximately in the range [-1, 1].
//------------------------------------------------------------------------------
float ULandmassGenerator::Noise2D(float X, float Y) const
{
	float Result = 0.0f;
	SimplexNode->GenPositionArray2D(&Result, 1, &X, &Y, 0.0f, 0.0f, Settings.Seed);
	return Result;
}

//------------------------------------------------------------------------------
// FBM (Fractal Brownian Motion): Layered Simplex noise for natural-looking patterns.
// 
// Combines multiple "octaves" of Simplex noise at increasing frequencies and
// decreasing amplitudes via the pre-built SimplexNode (thread-safe).
// Each octave uses a different seed offset (Seed + i * 31) to decorrelate layers.
// 
// Parameters:
...
float ULandmassGenerator::FBM(float X, float Y, int32 Octaves, float Persistence) const
{
	...
	for (int32 i = 0; i < Octaves; ++i)
	{
		...
		float N = 0.0f;
		SimplexNode->GenPositionArray2D(&N, 1, &SampleX, &SampleY, 0.0f, 0.0f, Settings.Seed + i * 31);
		Total += N * Amplitude;
